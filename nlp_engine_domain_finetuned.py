...
from advanced_marker_loader_llm_judge import (
    load_advanced_marker_lookup,
    HybridLLMJudge,
)
...
class SpellingCorrectionModule:
    ...
    def correct_word(self, word):
        ...
        # Skip acronyms (all uppercase, 2-5 chars)
...
class TemporalContextDetector:
    ...
    def get_temporality(self, entity_start, full_text, window=100):
        ...
        # Also look at the whole sentence
        sentence_start = full_text.rfind('.', 0, entity_start)
...
LABELED_PATTERNS = {
    ...
    'glucose_labeled': (r'(?:glucose|fasting\s*glucose|blood\s*sugar)\s*[:=]?\s*(\d{2,4})(?:\s*mg/dl)?', 'lab_test', 0.96, 'glucose'),
    ...
    'calcium_labeled': (r'(?:calcium|ca)\s*[:=]?\s*(\d{1,2}(?:\.\d+)?)\s*(?:mg/dl)?', 'lab_test', 0.96, 'calcium'),
...
class MorphologicalAnalyzer:
    ...
    def decompose(self, word):
        ...
        suffixes = ['itis', 'osis', 'emia', 'penia', 'pathy',
...
class SemanticAnalyzer:
    ...
    def compute_semantic_support(self, entities):
...
class DerivationEngine:
    ...
    def derive_numeric_conditions(self, entities):
        ...
        for e in entities:
            ...
            has_arrhythmia_context = any(k in context for k in ['atrial fibrillation', 'afib', 'a-fib', 'rapid ventricular response'])
            ischemic_context = any(k in context for k in ['st elevation', 'stemi', 'nstemi', 'troponin', 'myocardial infarction'])

            if 'bp_complete_sys' in name:
                if val < 90:
            ...
            if 'hr_labeled' in name or 'pulse_hr' in name:
                ...
                elif val >= 110 and (has_tachy_word or ischemic_context or has_shock_context or has_infectious_context):
                    add_label('tachycardia', 0.94, f"HR={val}")
                elif val > 100 and (has_tachy_word or ischemic_context or has_shock_context):
...
class DomainSpecificEngine:
    ...
    def __init__(self):
        ...
        # --- NEW: Load advanced markers and LLM Judge ---
        ...
        self.negation_detector = ScopedNegationDetector()
        ...
        #stage8
        self.llm_judge = HybridLLMJudge(use_llm=False)
          
        total_patterns = len(self.patterns) + len(self.labeled_patterns)
        print(f"[INIT] Domain-Specific Engine v8 initialized:")
        print(f"  - {sum(len(v) for v in self.marker_dict.values())} clinical markers")
        print(f"  - {total_patterns} domain-validated regex patterns")
        print(f"  - {len(CLINICAL_CONTEXT)} clinical context rules")
        print(f"  - {len(SNOMED_CT_MAPPINGS)} SNOMED CT concept mappings")
        print(f"  - Morphological Analysis: [YES]")
        print(f"  - POS Tagging & Syntactic Analysis: [YES]")
        print(f"  - Semantic Analysis: [YES]")
        print(f"  - Pragmatic Analysis: [YES]")
        print(f"  - SNOMED CT Normalization: [YES]")
        print(f"  - Scoped Negation Detection: [YES]")
        print(f"  - Spelling Correction: [YES]")
        print(f"  - Temporal Context Detection: [YES]")
        print(f"  - Derivation Engine: [YES]")
        print(f"  - Clinical Reasoning: [YES]")
        print(f"  - System: v8 Enterprise NLP with Medical Intelligence")
    ...
    def derive_morpheme_conditions(self, text, entities):
        """Derive candidate condition entities from common pathology morphemes (e.g., -itis)."""
        existing = {e.get('value', '').lower() for e in entities}
        additions = []
        for match in re.finditer(r'\b([A-Za-z]{5,}itis)\b', text):
            token = match.group(1).lower()
            if token in existing:
                continue

            # Require local clinical support before promoting raw -itis tokens.
            window = text[max(0, match.start() - 100):match.end() + 100].lower()
            has_support = any(
                e.get('value', '').lower() in window
                for e in entities
                if e.get('category') in ('lab_test', 'vital_sign', 'diagnosis')
            )
            if not has_support:
                continue
    ...
    def process(self, text, force_all_llm_judging=False):
        """
        ...
        # Apply Damerau-Levenshtein spelling correction (per Wulff 2020)
        processed_text = self.spelling_corrector.correct_text(text)
        ...
        normalized = self.semantic_analyzer.compute_semantic_support(normalized)
        ...
        asserted_entities = filtered
        
        # --- NEW: Triggered LLM Judge to filter entities ---
        if hasattr(self, 'llm_judge') and self.llm_judge and self.llm_judge.use_llm:
            if force_all_llm_judging:
                # Evaluation fallback mode: run LLM judgment on all entities.
                asserted_entities = self.llm_judge.judge_entities(processed_text, asserted_entities)
            else:
                # Target high-FP-risk entities for LLM review:
                # dictionary-extracted symptom/diagnosis/condition items with no semantic support.
                needs_llm = []
                safe_entities = []

                for e in asserted_entities:
                    is_dict_extracted = e.get('source') == 'dictionary'
                    has_no_support = e.get('semantic_support', 'none') == 'none'
                    is_risky_category = e.get('category') in ('symptom', 'diagnosis', 'condition')

                    if is_dict_extracted and has_no_support and is_risky_category:
                        needs_llm.append(e)
                    else:
                        safe_entities.append(e)

                if needs_llm:
                    judged_entities = self.llm_judge.judge_entities(processed_text, needs_llm)
                    asserted_entities = safe_entities + judged_entities
                else:
                    asserted_entities = safe_entities
        # ----------------------------------------------------
        ...
        return direct_sorted + derived_sorted
